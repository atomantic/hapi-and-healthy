...
describe('Hapi-and-Healthy plugin: WARN state', function() {
    return;
    ...
    pluginConfigWarn.options.test.features = [
        function(cb){
            return cb(true, 'some feature or dependency is broken');
        }
    ];
    ...
    describe('Basic Configuration (WARN State)', function() {
    ...
    describe('Restricted Configuration', function() {
        ...
        it('should load restricted plugin succesfully', function(done){
            ...
            server.stop(function () {
                ...
                // remove usage data
                pluginConfigWarn.options.usage = false;
                server.pack.register(pluginConfigWarn,
                function(err) {
                    expect(err).to.equal(undefined);
                    done();
                });
